...
int read_ov8856qt_AF_info(void)
{
   ...
   af_data=af_data_info[i];
             LOG_INF("lujun_ov8856_qunhui_otp_read_test  af_data_info: 0x%x,\n",  af_data); 
   }
...
ov8856qt_eeprom_data.Data[20]=((((af_data_info[1]<<2)+((af_data_info[2]>>4)&0x03))>>8)&0x03);
   LOG_INF("lujun_ov8856_qunhui_otp_read_test  data[17]: 0x%x,\n",  ov8856qt_eeprom_data.Data[17]); 
   LOG_INF("lujun_ov8856_qunhui_otp_read_test  data[18]: 0x%x,\n", ov8856qt_eeprom_data.Data[18]); 
   LOG_INF("lujun_ov8856_qunhui_otp_read_test  data[19]: 0x%x,\n", ov8856qt_eeprom_data.Data[19]); 
   LOG_INF("lujun_ov8856_qunhui_otp_read_test  data[20]: 0x%x,\n", ov8856qt_eeprom_data.Data[20]); 
   }	
   return 0;
}
#if 1
...
do_gettimeofday(&ktv2);
	if(ktv2.tv_sec > ktv1.tv_sec)
        ...
        printk("lujun>>ov8856_qunhui>read data take %lu us\n", TimeIntervalUS);
	return 1;
}
...
static long ov8856qt_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    stCAM_CAL_INFO_STRUCT __user *data;
    int err;
	CAM_CALDB("[CAMERA SENSOR] ov8856_qunhui_eeprom_DEVICE_ID,%p %p %x ioc size %d\n",filp->f_op ,filp->f_op->unlocked_ioctl,cmd,_IOC_SIZE(cmd) );

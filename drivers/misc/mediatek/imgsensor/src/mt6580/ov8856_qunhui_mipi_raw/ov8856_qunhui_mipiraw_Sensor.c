 ...
 * Filename:
 * ---------
 *	 OV8856_qunhui_mipiraw_sensor.c
 *
 * Project:
 * --------
 *	 ALPS MT6795
...
#include "ov8856_qunhui_mipiraw_Sensor.h"

#define PFX "OV8856_QUNHUI"
...
typedef enum {
	OV8856_QUNHUI_R2A,
	OV8856_QUNHUI_R1A
}OV8856_QUNHUI_VERSION;

OV8856_QUNHUI_VERSION ov8856_qunhui_version = OV8856_QUNHUI_R1A;

static DEFINE_SPINLOCK(imgsensor_drv_lock);

static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV8856_QUNHUI_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
static int read_otp(struct otp_struct *otp_ptr)
{
   ...
   // OTP base information and WB calibration data
   otp_flag = read_cmos_sensor(0x7010);
   LOG_INF("ov8856_qunhui_otp_read_test  otp_flag: 0x%x,\n", otp_flag); 
   addr = 0;
   ...
   LOG_INF("ov8856_qunhui_otp_read_test  (*otp_ptr).module_integrator_id: 0x%x,\n", (*otp_ptr).module_integrator_id); 
   // OTP VCM Calibration
   otp_flag = read_cmos_sensor(0x7023);
   
   LOG_INF("ov8856_qunhui_otp_read_test  otp_flag_2: 0x%x,\n", otp_flag); 
   addr = 0;
   if((otp_flag & 0xc0) == 0x40) 
   {
 ...
 LOG_INF("ov8856_qunhui_otp_read_test	otp_flag_3: 0x%x,\n", otp_flag); 
   addr = 0;
   int checksum2=0;
   ...
   if(addr != 0) 
   {
     ...
     checksum2 = (checksum2)%255 +1;
	  (* otp_ptr).checksum = read_cmos_sensor((addr + 240));
        LOG_INF("ov8856_qunhui_otp_read_test  checksum2: 0x%x,\n",checksum2 );
	   LOG_INF("ov8856_qunhui_otp_read_test  (* otp_ptr).checksum : 0x%x,\n",(* otp_ptr).checksum );
		...
		do {
			...
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			...
			if (module_id==0X06)
		     ...
		     LOG_INF("OV8856_QUNHUI sensor id: 0x%x\n", *sensor_id);
		     LOG_INF("OV8856_QUNHUI  imgsensor_info_sensor id: 0x%x\n", imgsensor_info.sensor_id);
		     LOG_INF("OV8856_QUNHUI module id: 0x%x\n",module_id);
			...
			//if ((*sensor_id == imgsensor_info.sensor_id)) {
				...
				#if HQ_ANTI_CAMERA_INTERACT
				 	g_CurrUsedMainCameraName = MAIN_OV8856_QUNHUI;
				#endif	
				
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x, ov8856_qunhui_version = %d(0=r2a,1=r1a)\n", imgsensor.i2c_write_id,*sensor_id,ov8856_qunhui_version);	
				return ERROR_NONE;
		 }	
		}
...
UINT32 OV8856_QUNHUI_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{

...
OV8856_VERSION ov8856_huaquan_version = OV8856R1A;

static DEFINE_SPINLOCK(imgsensor_drv_lock);

static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV8856_HUAQUAN_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
static int read_otp(struct otp_struct *otp_ptr)
{
   ...
   else 
   {
    ...
    LOG_INF("ov8856_otp_read_test  (* otp_ptr).VCM_start: 0x%x,(* otp_ptr).VCM_end: 0x%x\n", (* otp_ptr).VCM_start,(* otp_ptr).VCM_end); 
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
   ...
   /*
    ...
    LOG_INF("[imgsensor] ov8856 get_imgsensor_id\n");
			...
			if(0x8800==(read_cmos_sensor(0x300B) << 8)){
				...
				*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
				...
				if ((*sensor_id == imgsensor_info.sensor_id)&&(module_id==0X08)){
                    ...
                    read_otp(&otp_ptr);//sensor otp,read only one time
                    read_ov8856_eeprom_data();//platform otp, read only one time

					#if HQ_ANTI_CAMERA_INTERACT
				 		g_CurrUsedMainCameraName = MAIN_OV8856_HUAQUAN;
					#endif	
				
					LOG_INF("i2c write id: 0x%x, sensor id: 0x%x, ov8856version = %d(0=r2a,1=r1a)\n", imgsensor.i2c_write_id,*sensor_id,ov8856_huaquan_version);	
					return ERROR_NONE;
				}
			}
...
static kal_uint32 open(void)
{
    ...
    //read_otp(&otp_ptr);
	apply_otp(&otp_ptr);//sensor otp apply when opening sensor every time 
	//	kfree(otp_ptr);
	#endif
	spin_lock(&imgsensor_drv_lock);
...
UINT32 OV8856_HUAQUAN_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{

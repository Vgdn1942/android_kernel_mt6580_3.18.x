...
#define LTR553_DEV_NAME   "LTR_553"
...
static int ltr553_als_read(int gainrange)
{
	...
	if (luxdata_int < (als_level[current_tp][current_color_temp][3] - 50)) {
		if (als_gainrange != ALS_RANGE_600) {
			ltr553_i2c_write_reg(LTR553_ALS_CONTR, MODE_ALS_ON_Range6);
		...
		if (als_gainrange != ALS_RANGE_64K) {
			ltr553_i2c_write_reg(LTR553_ALS_CONTR, MODE_ALS_ON_Range1);
...
static int ltr553_init_device(void)
{
	...
	error = ltr553_i2c_write_reg(LTR553_PS_LED, 0x7F);
	if (error < 0) {
		APS_LOG("ltr553_ps_enable() error3...\n");
		return error;
	}

	error = ltr553_i2c_write_reg(LTR553_PS_N_PULSES, 0x4);	/* modify by hzb */
	if (error < 0) {
		       ...
		       void *buff_out, int size_out, int *actualout)
{
	...
	case SENSOR_GET_DATA:
		if ((buff_out == NULL) || (size_out < sizeof(struct hwm_sensor_data))) {
			...
			sensor_data->values[0] = ltr553_get_als_value(obj, obj->als);	/* wisky-lxh@20150206 */
#else
...
static int als_get_data(int *value, int *status)
{
	...
	*value = ltr553_get_als_value(obj, obj->als);
	*status = SENSOR_STATUS_ACCURACY_MEDIUM;
#endif				/* #ifdef CUSTOM_KERNEL_SENSORHUB */
	mutex_unlock(&Ltr553_lock);
	return err;

...
s32 init_wr_node(struct i2c_client *client)
{
	...
	if (i) {
		DATA_LENGTH1 = i * DATA_LENGTH_UINT + GTP_ADDR_LENGTH;
		GTP_INFO("Applied memory size:%d.", DATA_LENGTH1);
	} else {
...
static ssize_t goodix_tool_write(struct file *filp, const char __user *buff, size_t len, loff_t *off)
{
	...
	if(cmd_head.data_len > DATA_LENGTH1 - 2*GTP_ADDR_LENGTH)
		return -EFAULT;
	if (1 == cmd_head.wr) {
		...
		if(cmd_head.data_len > sizeof(IC_TYPE)/sizeof(IC_TYPE[0]))
			cmd_head.data_len = sizeof(IC_TYPE)/sizeof(IC_TYPE[0]);
